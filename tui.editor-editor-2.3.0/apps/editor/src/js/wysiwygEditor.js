...
class WysiwygEditor {
  ...
  _initDefaultKeyEventHandler() {
    ...
    this.addKeyEventHandler('ESCAPE', (ev, range, keymap) =>
      this._handleEscapeKeyEvent(ev, range, keymap)
    );
  }
  ...
  _handleEscapeKeyEvent(ev, range, keyMap) {
    const sq = this.getEditor();
    const editorEl = sq.getRoot();
    this._getNextExternalFocusableElement(editorEl).focus();
  }

  _getNextExternalFocusableElement(editorElement) {
    const externalElements = this._getExternalFocusableElements(editorElement);

    for (let el of externalElements) {
      if (this._compareDocumentPosition(editorElement, el) < 0) {
        return el;
      }
    }
    return externalElements[0] || null;
  }

  _getExternalFocusableElements(editorElement) {
    ...
    const allFocusable = Array.from(document.querySelectorAll(focusableElements.join(','))).filter(
      el => el.offsetParent !== null
    );

    return allFocusable.filter(el => el !== editorElement && !editorElement.contains(el));
  }
  _compareDocumentPosition(a, b) {
